...
void CVideoPlayer::GetGeneralInfo(std::string& strGeneralInfo)
{
  ...
  if (!m_bStop && (m_playSpeed == DVD_PLAYSPEED_NORMAL))
  {
    ...
    strGeneralInfo = StringUtils::Format("P: a/v~:{: 6.3f}, af/vf~:{: 6.3f}, a/c~:{: 6.3f}, v/c~:{: 6.3f}, af/c~:{: 6.3f}, vf/c~:{: 6.3f}, {}",
        dDiffDeltaMovingAverage, dDiffDeltaFrameMovingAverage,
        dDiffAudioMovingAverage, dDiffVideoMovingAverage,
        dDiffAudioFrameMovingAverage, dDiffVideoFrameMovingAverage, 
        strBuf);
  }
  else if (!resetDone)
  {

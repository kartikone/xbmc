...
void CRenderManager::PrepareNextRender()
{
  ...
  m_displayLatency = DVD_MSEC_TO_TIME(-m_videoDelay);
  ...
  if (normalPlayback)
  {
    // Allow up to 22ms behind before skipping frames.
    while ((diff > 22000) && (m_queued.size() > 2))
    {
      m_QueueSkip++;
    ...
    if ((diff < 0) && (diff > -1000000) && (m_presentpts > m_displayLatency))
    {
      usleep(-diff);

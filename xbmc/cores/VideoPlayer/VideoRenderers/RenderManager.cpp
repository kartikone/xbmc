  ...
  StreamHdrType hdrType, int buffers)
{
  ...
  logM(LOGDEBUG,"CRenderManager", "change configuration. {}x{}. display: {}x{}. framerate: {:4.2f} hdrType: {}.",
            picture.iWidth, picture.iHeight,
            picture.iDisplayWidth, picture.iDisplayHeight,
            fps, CStreamDetails::DynamicRangeToString(hdrType));
  ...
  {
    logM(LOGWARNING, "CRenderManager", "timeout waiting for configure");
    std::unique_lock<CCriticalSection> lock(m_statelock);
    return false;
  }
  ...
  {
    logM(LOGWARNING, "CRenderManager", "failed to configure");
    return false;
  }
...
bool CRenderManager::Configure()
{
  ...
  if (result)
  {
    ...
    logM(LOGDEBUG, "CRenderManager", "queue size {}", m_QueueSize);
  }
  else
...
void CRenderManager::PreInit()
{
  ...
  if (!CServiceBroker::GetAppMessenger()->IsProcessThread())
  {
    ...
    {
      logM(LOGERROR, "CRenderManager", "timed out waiting for renderer to preinit");
    }
  }
...
void CRenderManager::UnInit()
{
  if (!CServiceBroker::GetAppMessenger()->IsProcessThread())
  {
    ...
    if (!m_initEvent.Wait(2000ms))
    {
      logM(LOGERROR, "CRenderManager", "timed out waiting for renderer to uninit");
    }
  }
...
bool CRenderManager::Flush(bool wait, bool saveBuffers)
{
  ...
  if (CServiceBroker::GetAppMessenger()->IsProcessThread())
  {
    logM(LOGDEBUG, "CRenderManager", "flushing renderer");
...
// fix deadlock on Windows only when is enabled 'Sync playback to display'
  ...
  else
  {
    ...
    if (wait)
    {
      ...
      {
        logM(LOGERROR, "CRenderManager", "timed out waiting for renderer to flush");
        return false;
      }
      else
        return true;
...
void CRenderManager::StartRenderCapture(unsigned int captureId, unsigned int width, unsigned int height, int flags)
{
  ...
  if (it == m_captures.end())
  {
    logM(LOGERROR, "CRenderManager", "Capture - unknown capture id: {}", captureId);
    return;
  }
  ...
  if (gui)
  {
    ...
    if (m_renderDebug)
    {
      ...
      else
      {
        ...
        int missedvblanks;
        info.vsync = StringUtils::Format("VSyncOff: {:.1f} vt:{:.3f}",
...
void CRenderManager::UpdateVideoLatencyTweak()
{
  ...
  m_videoLatencyTweak = static_cast<int>(CServiceBroker::GetSettingsComponent()
                                                        ->GetAdvancedSettings()
                                                        ->GetVideoLatencyTweak(refresh, res.iScreenHeight));
}
  ...
  if (wait)
  {
    ...
    while (m_presentstep == PRESENT_READY)
    {
      ...
      if(endtime.IsTimePast() || bStop)
      {
        if (!bStop)
        {
          logM(LOGWARNING, "CRenderManager", "timeout waiting for render");
        }
        break;
      }
    }
...
void inline CRenderManager::DiscardPresentSource()
{
  if (m_discard.empty() || (m_discard.back() != m_presentsource))
    m_discard.push_back(m_presentsource);
}

void inline CRenderManager::BusyWait(useconds_t uSeconds)
{
  struct timespec start, now;
  clock_gettime(CLOCK_MONOTONIC, &start);
  long wait_ns = uSeconds * 1000; // Convert microseconds to nanoseconds

  do {
    clock_gettime(CLOCK_MONOTONIC, &now);
  } while ((((now.tv_sec - start.tv_sec) * 1000000000L) + (now.tv_nsec - start.tv_nsec)) < wait_ns);
}
...
void CRenderManager::PrepareNextRender()
{
  ...
  if (m_presentstarted) DiscardPresentSource();
  ...
  bool normalPlayback = (speed == 1.0f);
  ...
  if (normalPlayback)
  {
    if (diff > 0)
      while ((diff > -1000) && (m_queued.size() > 2))
      {
        ...
        m_queued.pop_front();
        DiscardPresentSource();
        SetPresentSource(); // Get next frame
        diff = (renderPts - m_presentpts);
      }

    delay = diff; // keep delay for logging.
  ...
  if ((normalPlayback && (diff >= 0)) ||
      m_forceNext ||
      ((speed != 0.0f) && (m_queued.size() > 2)))
  {
  ...
  logM(LOGINFO, "CRenderManager",
                ...
                m_queued.size(), (frametime / DVD_TIME_BASE), m_QueueSkip, m_forceNext, speed);
}
